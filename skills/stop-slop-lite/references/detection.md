...
#### Bone / Cream Palette Detection (Opus 4.7/4.8 house style)

The 2026 replacement for VibeCode Purple. Detect the combination of: warm-cream background + warm-charcoal text + terracotta/sage/dust-rose accent. All three must be present — the palette is the tell, not any single color.

```javascript
// Check: Bone/cream background + warm-charcoal text + terracotta/sage accent
function detectBonePalette(ctx) {
  const bodyStyle = ctx.getComputedStyle(ctx.document.body);
  const bodyBg = parseHex(bodyStyle.backgroundColor);
  if (!bodyBg) return false;

  // Bone background anchors (±6 in each channel)
  const boneAnchors = [
    [0xF4, 0xF1, 0xEA], // #F4F1EA — Anthropic Opus 4.8 docs
    [0xF4, 0xF3, 0xEE], // #F4F3EE — Claude brand
    [0xF6, 0xF5, 0xF2], // #F6F5F2 — Mercury
    [0xFA, 0xF8, 0xF2], // #FAF8F2 — Granola
    [0xFA, 0xF7, 0xF2],
    [0xF0, 0xEB, 0xE3], // Alabaster
  ];
  const isBone = boneAnchors.some(([r, g, b]) =>
    Math.abs(bodyBg.r - r) <= 6 &&
    Math.abs(bodyBg.g - g) <= 6 &&
    Math.abs(bodyBg.b - b) <= 6
  );
  if (!isBone) return false;

  // Warm-charcoal text (#191817 through #37352F range)
  const textCol = parseHex(bodyStyle.color);
  if (!textCol) return false;
  const isWarmCharcoal =
    textCol.r < 70 && textCol.g < 70 && textCol.b < 70 &&
    textCol.r >= textCol.g && textCol.g >= textCol.b;
  if (!isWarmCharcoal) return false;

  // Accent: terracotta (#C96442 ±), sage (#8A9A7B ±), dust rose (#C9A8A0 ±), amber (#C98A42 ±)
  const accents = ctx.document.querySelectorAll('a, button, [class*="accent"], [class*="primary"], [class*="cta"]');
  for (const el of accents) {
    const s = ctx.getComputedStyle(el);
    const col = parseHex(s.color) || parseHex(s.backgroundColor);
    if (!col) continue;
    // Terracotta range
    if (col.r > 180 && col.r < 230 && col.g > 90 && col.g < 130 && col.b > 50 && col.b < 90) return true;
    // Sage range
    if (col.r > 130 && col.r < 160 && col.g > 150 && col.g < 175 && col.b > 115 && col.b < 140) return true;
    // Dust rose range
    if (col.r > 195 && col.r < 215 && col.g > 160 && col.g < 180 && col.b > 155 && col.b < 175) return true;
  }
  return false;
}
```

#### Italic Word-Accents in Headings (Opus 4.8 copy pattern)

The Opus 4.8 prompt guide names this verbatim: "italic word-accents." Detects `<em>` tags inside headings, especially when paired with serif font-family.

```javascript
// Check: <em> inside h1/h2/hero, especially with serif font
function detectItalicWordAccents(ctx) {
  const headings = ctx.document.querySelectorAll('h1, h2, [class*="hero"], [class*="headline"]');
  for (const h of headings) {
    const ems = h.querySelectorAll('em, i');
    if (ems.length === 0) continue;
    const ff = ctx.getComputedStyle(h).fontFamily.toLowerCase();
    const isSerif = ff.includes('serif') || ff.includes('fraunces') ||
                    ff.includes('playfair') || ff.includes('georgia') ||
                    ff.includes('instrument');
    // 1+ em tag with serif font, OR 2+ em tags regardless of font
    if (isSerif && ems.length >= 1) return true;
    if (ems.length >= 2) return true;
  }
  return false;
}
```
