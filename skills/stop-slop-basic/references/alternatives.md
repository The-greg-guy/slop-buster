# Design Slop Alternatives (Advanced)

What to use instead of the default AI patterns, with code examples and framework guidance.
...
**Rule:** Pair a distinctive display font with a refined body font. Two fonts minimum. Three maximum. The contrast should be immediately visible.

### Letter-Spacing Guide

| Element | Spacing | CSS | Rationale |
|---------|---------|-----|-----------|
| Large headings (48px+) | -0.01 to -0.02em | `letter-spacing: -0.02em` | Pulls letters together. Confident, dense. |
| Sub-headings (24–40px) | -0.005 to -0.01em | `letter-spacing: -0.01em` | Slight tightening. Clean. |
| Body text (14–18px) | 0 (default) | `letter-spacing: normal` | Natural reading. Don't interfere. |
| Small labels (11–13px) | 0.04 to 0.06em | `letter-spacing: 0.05em` | Slightly open. Reads as a label. |
...
| Palette | Background | Surface | Text | Accent | Secondary | Feel |
|---------|-----------|---------|------|--------|-----------|------|
| Earth | #F5E6C4 | #FFF8F0 | #2D2926 | #8B6914 | #5C4A1E | Warm, grounded |
| Coral | #F7F3F0 | #FFFFFF | #2C2825 | #E8836B | #C4493C | Refined, warm |
| Ink | #F5F0EB | #FFFFFF | #1C1917 | #C4493C | #1C1917 | Editorial, classic |
| Forest | #F0F5F1 | #FFFFFF | #1A1A2E | #4A7C5C | #1A3C2A | Natural, trustworthy |
| Sunset | #1A1A2E | #252540 | #F5F5F5 | #FF6B35 | #F7C59F | Energetic, bold |
| Cool | #FFFFFF | #F5F5F5 | #0A0A0A | #0066FF | #003380 | Confident, technical |
| Cream | #FFF8F0 | #FFE8D6 | #2D2926 | #E85D4A | #D4A373 | Friendly, human |
| Midnight | #0F1419 | #1A2332 | #E7E9EA | #1D9BF0 | #8B98A5 | Dark, professional |
...
| Muted text | #666666 | #A1A1AA, still passes for large text (3:1) |
...
### Instead of the Default Template (Hero → Metrics → Features → Testimonials → Pricing → CTA)
...
| Asymmetric hero | Left text + right visual (or reverse) → Alternating features → CTA | Landing pages |
| Comparison layout | Before/after or competitor comparison → Feature grid → CTA | Competitive products |
...
| Featured + list | One large featured item + smaller items. | Flexbox with `flex: 2` on featured, `flex: 1` on others. |
| Side-by-side | Icon and text on the same line, not stacked. | Flexbox row with `align-items: center`. |
| Alternating sections | Feature sections alternate left-right with different treatments. | Odd/even section styling. |
...
| Alternative | CSS |
|-------------|-----|
| No border | Remove `border-left`. Just spacing. |
...
| Left padding + vertical rule | `border-left: 1px solid var(--color-border); padding-left: 1rem` |
...
| Embedded demo | Show the actual process (e.g., terminal, video). |
...
3. **50–100ms stagger** between elements. Not uniform delay.
4. **300–600ms duration** for entrances. Not 2-second sagas.
...
|------|-------------|-----|
| "Build the Future" | "Ship code 3× faster with AI pair programming" | Specific claim, specific mechanism |
| "Transform Your Workflow" | "Replace 5 tabs with 1 command palette" | Concrete before/after |
