...
name: ponytail
version: 1.0.0
description: Lazy senior dev mode. Forces the simplest, shortest solution that works. YAGNI, stdlib first, no unrequested abstractions. Loaded as a peer to design-slop and text-slop within Stop Slop Ultimate.
metadata:
...
- **Abstraction with one current implementation.** Defense required: "The abstraction is requested or it serves two or more current needs." Speculative abstraction does not qualify.
- **Adding a dependency.** Defense required: "The dependency solves a problem stdlib and existing deps cannot, and the cost of ownership is lower than the cost of the code it replaces." Always prefer a few lines of your own code to a new dep.
- **Boilerplate for later.** Defense required: "The boilerplate is for a feature on the immediate roadmap, not speculative." Later can scaffold for itself.
- **Function longer than 20 lines.** Defense required: "The function does one thing and splitting it would make it harder to read." Most long functions do more than one thing. Split them.
